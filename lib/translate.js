...
- Use the batch version as the base (better structure/punctuation)
- When the realtime version contains content the batch version dropped, include it
...
async function tryMergeAnalyze(realtimeText, batchText, matchContext) {
  ...
  const userMessage = `Realtime transcription:\n"${realtimeText}"\n\nBatch transcription:\n"${batchText}"`;
  ...
  try {
    ...
    console.error(`[capito] mergeAndAnalyze failed: ${err.message}`);
    return null;
  }
}
...
async function mergeAndAnalyze(realtimeText, batchText, matchContext) {
  for (let attempt = 0; attempt < 2; attempt++) {
    const result = await tryMergeAnalyze(realtimeText, batchText, matchContext);
    if (result) return result;
    if (attempt === 0) console.log('[capito] mergeAndAnalyze retry...');
  }
  return null;
}

export { analyzeCommentary, splitAndAnalyze, mergeAndAnalyze };
