...
1. "segments": split the Italian text at natural clause boundaries (commas, conjunctions, natural pauses) into chunks of roughly 5-15 words each. Return an array of {it, en} pairs where "it" is the Italian chunk (with player/team names corrected to their proper spelling using match context) and "en" is its English translation. Each English chunk should read naturally. Avoid segments shorter than 3 words — merge short fragments with the adjacent clause.

2. "translation": the full English translation as a single string

3. "entities": array of named entities [{text, type}] where type is "player"|"team"|"stadium"|"coach". IMPORTANT: "text" must be the SHORT name exactly as it appears in your corrected segment "it" text (e.g. use "Anguissa" not "Andre-Frank Zambo Anguissa", use "Milan" not "AC Milan"). Only include entities that actually appear in the segments text. Do NOT hallucinate entities that aren't mentioned. Only include entities whose names appear in the match context (if provided). Do not guess players who aren't in the squad list.
