...
function buildContextString(data) {
  ...
  if (data.managers) {
    ...
    if (Array.isArray(data.managers)) {
      mgrs = data.managers.map(m => typeof m === 'string' ? m : `${m.name} (${m.team})`);
    } else if (typeof data.managers === 'object') {
      mgrs = Object.entries(data.managers).map(([role, name]) => `${name} (${role})`);
    }
    if (mgrs?.length) lines.push(`Managers: ${mgrs.join(', ')}`);
    handled.add('managers');
  }
  if (data.teams?.length) {
    lines.push('');
    for (const team of data.teams) {
      const squad = team.squad || team.starting_XI || team.starting_xi || team.players || [];
      const bench = team.bench;
      const squadStr = Array.isArray(squad) ? squad.join(', ') : String(squad);
      lines.push(`${team.name}: ${squadStr}`);
      if (Array.isArray(bench) && bench.length) {
        lines.push(`${team.name} (bench): ${bench.join(', ')}`);
      }
    }
    handled.add('teams');
  }
