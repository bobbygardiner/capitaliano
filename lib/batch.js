...
export function createBatchPipeline(options) {
  const { contextBias, onUpgrade, transcribeFn, analyzeFn, splitAnalyzeFn, getBiasTokens } = options;
  ...
  async function submitBatch(audioBuffer, lineIds, originalTexts) {
    ...
    const promise = (async () => {
      try {
        ...
        // Quality gate: skip upgrade if batch has fewer recognized bias tokens.
        // getBiasTokens is an optional callback (set by server.js) that returns
        // the current context_bias token list for comparison.
        if (getBiasTokens) {
          const tokens = getBiasTokens();
          if (tokens.length > 0) {
            const origText = originalTexts.join(' ').toLowerCase();
            const batchText = batchResult.text.toLowerCase();
            const origHits = tokens.filter(t => origText.includes(t.toLowerCase())).length;
            const batchHits = tokens.filter(t => batchText.includes(t.toLowerCase())).length;
            if (origHits > 0 && batchHits < origHits) {
              console.log(`[capito] Quality gate: skipping upgrade for lines ${lineIds.join(',')} (batch ${batchHits} vs original ${origHits} entity matches)`);
              return;
            }
          }
        }
